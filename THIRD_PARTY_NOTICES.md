...
11. **移除主菜单中的四项** — 应用商店（引导下载手机 App）、分享（外链社交）、夜间模式（改为跟随 Obsidian 的深/浅色设置）、声音开/关（本移植静音分发，`AudioEngine` 是 no-op，该开关不会有任何反应）。构建期按块删除对应创建代码；`Engine.getApp` / `Engine.share` / `Engine.turnLightsOff` 函数本身保留（`turnLightsOff` 仍被启动逻辑与主题同步使用）。
...
14. **不动态加载任何外部脚本（jQuery 的跨域 script 传输被剥离）** — jQuery 1.10.1 内建一个「跨域 script 传输」：当 `$.ajax` 的 `dataType` 为 `script` 且请求跨域时，它会 `document.createElement("script")`、把 `s.url` 赋给 `src`、再插进 `<head>` —— 即运行期动态加载并执行任意外部脚本。这既是安全审计的必报项（*Code creates script elements at runtime*），对本插件也毫无用处：A Dark Room 全程不发跨域 script 请求（语言包已改为构建期「登记待用」、不落网络，见第 12 条），该分支是死代码。构建期把该 `x.ajaxTransport("script", …)` 调用整段替换为说明性注释，于是 `createElement("script")` 与 `n.src=e.url` 两处动态加载代码一并从产物中消失；此后若真有代码走到这条路径，jQuery 会因找不到对应 transport 而直接报错，而不是去加载远程脚本。与第 1 条（构建只取 `<body>` 骨架、忽略外链脚本）合起来，插件在运行期没有任何加载远程代码的路径。
